...
suppressPackageStartupMessages({
  library(monocle3) # from trapnell lab
  library(m3addon) # from furlan lab  manual install
  library(dplyr)
  library(Matrix)
  library(reticulate)
  library(ggplot2)
  ...
  library(viridis)
  library(parallel)
  library(scCustomize) #  manual install
  library(scRepertoire) #  manual install
  library(SCpubr) #  manual install
  library(forcats)
  library(ggalluvial) #  manual install with scRepertoire
  library(future)
  library(pbmcapply)
  library(msigdbr)
  library(fgsea)
  library(harmony) #  manual install
  library(circlize)
  library(scales)
  library(clusterProfiler) # For enricher function
  library(tibble)
  library(SeuratDisk) # needed for running scrublet in python
